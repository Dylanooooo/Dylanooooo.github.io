...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'add_project') {
    ...
    if (strtotime($_POST['eind_datum']) < strtotime($_POST['start_datum'])) {
        header('Content-Type: application/json');
        echo json_encode(['success' => false, 'message' => 'Einddatum moet na startdatum liggen']);
        exit();
    }
    ...
    if ($voortgang < 0 || $voortgang > 100) {
        header('Content-Type: application/json');
        echo json_encode(['success' => false, 'message' => 'Voortgang moet tussen 0 en 100 liggen']);
        exit();
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'add_task') {
    ...
    if (isset($_POST['deadline']) && !empty($_POST['deadline']) && !validateDate($_POST['deadline'])) {
        header('Content-Type: application/json');
    ...
    if (isset($_POST['toegewezen_aan']) && !empty($_POST['toegewezen_aan'])) {
        ...
        if ($stmt->rowCount() === 0) {
            header('Content-Type: application/json');
            echo json_encode(['success' => false, 'message' => 'Toegewezen gebruiker bestaat niet']);
            exit();
        }
    }
    ...
    try {
        ...
        $result = $stmt->execute([
            ...
            'status' => $_POST['status'],
            'toegewezen_aan' => !empty($_POST['toegewezen_aan']) ? $_POST['toegewezen_aan'] : null
        ]);
